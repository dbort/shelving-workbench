...
2. **Strict Step Isolation:** Break the code generation into separate files. Prefer steps where step $N$ does not depend on uncompleted files in step $N+1$. When a change is atomic across files (an interface rename, widening a type/lint gate) and a clean split is impossible, group those steps and place one deferred checkpoint line after the last of them (`.claude/docs/pipeline.md` § Deferred verification) rather than forcing a false split. Do not author a `tools/vendor-core.sh` step: the Implementer re-syncs the vendored core after every step that edits `shelving_core/` (`.claude/agents/implementer.md`).
