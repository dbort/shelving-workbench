...
3. Doc-comment convention: a function/type/module doc comment's summary line should lead with the identifier's name (where the language's convention does so) and add real information beyond repeating it. Keep parameter/return descriptions to one direct line. Move implementation-detail rationale out of function-level doc comments into a concise inline comment on the code block it actually explains (applies to test code too). A docstring that narrates the body step by step ("calls X, then emits Y, then returns Z") is describing implementation: replace it with the caller-facing contract (what the return guarantees, what it raises, which inputs have no effect). The internal call sequence belongs in the code, or in an inline comment where it is genuinely non-obvious.
