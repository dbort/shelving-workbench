...
- `2026-09-01` - **A mypy-checked test importing a path-listed helper hits "source file found twice"**: `[tool.mypy].files` lists `tools/*.py` helpers by path, so mypy names them as top-level modules (`check_action_pins`); the moment a test that is itself in `files` does `from tools.check_action_pins import ...`, mypy also names the same file `tools.check_action_pins` and refuses to run. Worked around by adding `explicit_package_bases = true` to `[tool.mypy]`. Simpler if: the repo checked helpers as `tools.<name>` from the start (a `tools/__init__.py`, or package-relative entries in `files`), so helper and test agree on the module name.
