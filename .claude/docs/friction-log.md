...
- `2026-09-03` - **`freecad-stubs` has no static surface for a scripted object's dynamic properties**: bringing `tools/freecad_object_smoke.py` into the `mypy --strict` gate (sh-011 review N3) meant every `doc.addObject(...)` return had to be `cast` to a hand-written `Protocol`, because the stub types `addObject` as a GUI proxy and a scripted object's `addProperty`-created fields (`SizeMM`, `Dimensions`, `Proxy`, ...) exist in no stub. Worked around by importing the private `_PlankFeature` Protocol from `freecad/shelving/objects/plank.py` into the tool script and adding a second local `_ProxyHolder` Protocol for the probe. Simpler if: the scripted-object Protocols lived in one shared, non-underscore module (e.g. `freecad/shelving/objects/_feature_types.py`) that both the workbench code and the headless checks import, instead of each call site re-declaring the property surface.
