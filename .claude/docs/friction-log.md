...
- `2026-09-02` - **sh-010 test case (d) asserts a physically inverted inequality**: the Execution Plan / Must Have for `test_expand.py` case (d) says a 12 mm `Divider.material` override "in a catalog whose default is 18mm" makes "a sibling leaf opening ... smaller than in the no-override case". With the shell always resolving to the 18 mm default, a thinner divider frees interior span, so the sibling opening comes out *larger*, not smaller. Implemented the test asserting `override_opening > baseline_opening` (the direction the solver actually produces) since the case's real intent, that the override lands on that plank, drops its thickness dimension to 12, and moves a sibling opening, is unambiguous and met. Simpler if: the plan stated the inequality in the direction the solver math produces, or named the reference case it meant.
