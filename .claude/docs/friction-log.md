...
- `2026-09-01` - **No GitHub token in the dev environment, so the online action-pin check flakes**: `pixi run tests` runs `tools/check-action-pins.sh`, which makes ~11 GitHub API calls per run. The dev VM has no `GH_TOKEN`/`GITHUB_TOKEN` and shares an IP under GitHub's 60-requests/hour anonymous limit, so a full-suite run mid-session returned HTTP 403 rate-limit fatals on `ossf/scorecard-action` and `github/codeql-action`. Worked around by reading `x-ratelimit-reset` and waiting ~40 minutes for the window to roll over before re-running the online verification (`--offline` and the mock-backed pytest cases are unaffected). Simpler if: the dev VM exported a read-only `GITHUB_TOKEN` the way `ci.yml` now passes `${{ github.token }}`, lifting the local quota to 1000/hour so the check does not flake between runs.
