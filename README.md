...
- **write_container**: reconciles a container's `Part::Box` children
  against a solved `Unit`, matched by document object `Name`. Updates a
  matching board in place, creates one with no match, and deletes only what
  it may (see "the deletion rule" below); returns what it updated, created,
  deleted, and left alone.
- **ShelvingMaterial / ShelvingBornAs / ShelvingBornIn / ShelvingIrregular**:
  the `Shelving` property group `write_container` writes on every board:
  the resolved catalog material id, the `Name` and document `Uid` it was
  first tagged under (see "the provenance rule" below), and whether it is
  geometrically irregular (pinned, per `Board.pinned_size_mm`).
- **ShelvingUnitId / ShelvingDepthAxis / ShelvingFacing / ShelvingRules**:
  the `Shelving` property group `write_container` writes on the container
  itself: the written `Unit.id`, its depth axis, its facing (`min` / `max`
  / `unknown`, a string enumeration since a FreeCAD property has no null),
  and the per-region size rule record from
  `freecad.Shelving.core.record.rules_to_json`.
- **the provenance rule**: `ShelvingBornAs` and `ShelvingBornIn` classify a
  mismatch rather than provide identity, which is the document object's own
  `Name`. `Name != ShelvingBornAs` means the object is a copy, adopted as
  new geometry rather than treated as a continuation of the board it was
  copied from; `ShelvingBornIn` differing from the document's own `Uid`
  alone means a relocation into a fresh document instead.
- **the deletion rule**: `write_container` deletes an object only when it
  carries the board property group AND the current `Unit` names no board at
  its `Name`. An object that matches a board is written regardless of
  whether it already carried the properties; one that matches nothing and
  carries none is left exactly alone.
