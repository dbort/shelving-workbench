...
headless guard so ``import shelving.commands.export_boxes`` succeeds
under ``freecadcmd``, where there is no GUI. This command never calls
``shelving.core.scan.scan``: it is the mechanism for capturing a unit that
...
from shelving.core.scan import ScanError
