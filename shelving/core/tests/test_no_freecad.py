"""Enforce the core invariant: :mod:`shelving.core` never pulls in FreeCAD.
...
def test_no_freecad_import_statements_in_source() -> None:
    ...
    assert not offenders, "FreeCAD imports found in shelving.core:\n" + "\n".join(
        offenders
    )


def test_importing_every_submodule_does_not_load_freecad() -> None:
    for mod in pkgutil.walk_packages(core.__path__, prefix="shelving.core."):
