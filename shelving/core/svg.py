...
:func:`shelving.core.solver.solve`), and a
:class:`~shelving.core.materials.Catalog` (for the material each board
...
that :mod:`shelving.core` stays free of runtime dependencies. One SVG user
...
from shelving.core.geometry import AxisIndex, Space, Vec3
from shelving.core.layout import (
    Axis,
    Basis,
    Bay,
    Board,
...
from shelving.core.materials import Catalog, MaterialId
...
def _elevation_axes(depth_axis: Axis) -> tuple[Axis, Axis]:
    ...
    :mod:`shelving.core.scan` uses to choose its grid, so a scanned unit's
...
def _apply_insets(axis: Axis, space: Space, insets: Insets) -> Space:
    ...
    never an inset. Mirrors :func:`shelving.core.expand._apply_insets`; kept
...
def to_svg(
    ...
    *,
    ...
    :func:`shelving.core.solver.solve`); a node id absent from it is a
