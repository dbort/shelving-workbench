...
:class:`~shelving.core.geometry.Space` placing one space per region
...
from shelving.core.materials import Catalog
...
def _place(
    ...
    """Record one :class:`~shelving.core.geometry.Space` per node id in the
...
def solve(unit: Unit, catalog: Catalog) -> dict[str, Space]:
    """One :class:`~shelving.core.geometry.Space` per region and board id.
